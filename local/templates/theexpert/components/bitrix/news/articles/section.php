...
if (isset($_REQUEST['designer']) && (int)$_REQUEST['designer'])
	$iRequestDesigner = (int)$_REQUEST['designer'];
else
	$iRequestDesigner = 0;
...
if ($arIBlock["PICTURE"])
{
	$pathIblockPicture = CFile::GetPath($arIBlock["PICTURE"]);
}
...
<!--                        <div class="topblock-header-note">Всего --><?//=$cnt?><!--</div>-->
        ...
        <div class="catalog-items-bordered container">
			...
			<div class="filters filters-sortonly">
                <div class="filters-sort">
                    Сортировать по:&emsp;<a href="">популярности <i class="icon icon-dropdown"></i></a>
                </div>
            </div>
...
//echo $iRequestDesigner;
if ($iRequestDesigner)
	$arrArticlesFilter = array("ACTIVE" => "Y", 'PROPERTY_DESIGNERS' => $arrArticlesFilter);
?>
<?$APPLICATION->IncludeComponent(
	"bitrix:news.list",
	"",
