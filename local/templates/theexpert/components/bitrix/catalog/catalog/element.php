...
const CollectionHighloadBlockId = 18;
...
<? if ($elementId > 0)
{
	...
	if (!empty($arCollectionProducts))
	{
		...
		{
			if (!in_array($arProduct['IBLOCK_SECTION_ID'], $arCollectionSections))
			{
				$arCollectionSections[] = $arProduct['IBLOCK_SECTION_ID'];
			}
		}
	}
	...
	{ ?>
	<!-- Collection -->
	<section class="product-promo collection">
        <div class="container">
			<div class="h1">
				<?=str_replace('{TITLE}', $collectionName, GetMessage('CATALOG_COLLECTION'))?>
			</div>
			<div class="tabs">
			...
			<?$APPLICATION->IncludeComponent(
				...
				array(
					...
					"PRODUCT_QUANTITY_VARIABLE" => $arParams["PRODUCT_QUANTITY_VARIABLE"],
					"PRODUCT_PROPS_VARIABLE" => $arParams["PRODUCT_PROPS_VARIABLE"],
					"FILTER_NAME" => $filterName,//$arParams["FILTER_NAME"],
					"CACHE_TYPE" => $arParams["CACHE_TYPE"],
					"CACHE_TIME" => $arParams["CACHE_TIME"],
	...
	<section class="recommended-section">
		<div class="container">
			...
			while($ob = $res->GetNextElement())
			{
				...
				if($arFields["PROPERTY_RECOMMEND_VALUE"]){
				 $iddArr[] .= $arFields["PROPERTY_RECOMMEND_VALUE"];
				}
			}

			if (!empty($elementId) && count($iddArr) > 0)
			{?>
			<div class="" data-entity="parent-container">
				<div class="h1 product-promo-title" data-entity="header" data-showed="false" style="display: none; opacity: 0;"><?=GetMessage('CATALOG_RECOMMENDED_BY_LINK')?></div>
				<div class="product-promo-carousel">
					<?
	...
	<section class="delivery">
		...
		<?$APPLICATION->IncludeComponent(
			...
			Array(
				...
				"AREA_FILE_SUFFIX" => "",
				"EDIT_TEMPLATE" => "",
				"PATH" => SITE_TEMPLATE_PATH . "/includes/catalog/delivery_pay.php"
			)
		);?>
		</div>
	</section>
    <!-- Question -->
    <section class="question product-question">
        <div class="container">
            <div class="question-header">
                <div class="h2 question-title">Задать вопрос о продукции</div>
...
<? if ($elementId > 0)
    ...
    <section class="product-promo">
		...
		<?
				...
				?>
			<div class="" data-entity="parent-container">
				<div class="h1 product-promo-title" data-entity="header" data-showed="false" style="display: none; opacity: 0;"><?=GetMessage('CATALOG_POPULAR_IN_SECTION')?></div>
				<?
							$APPLICATION->IncludeComponent(
								'bitrix:catalog.section',
								'viewed',
				...
				?>
			<div class="" data-entity="parent-container">
				<div class="h1 product-promo-title" data-entity="header" data-showed="false" style="display: none; opacity: 0;"><?=GetMessage('CATALOG_VIEWED')?></div>
				<div class="product-promo-carousel carousel js-carousel-three owl-carousel">
					<?
							$APPLICATION->IncludeComponent(
...
<?if($_REQUEST["set_filter"]){?>
    <script type="text/javascript">
        $(document).ready(function(){
            window.scrollTo($(".catalog-item").scrollTop(), 1000 );
        });
    </script>
<?}?>