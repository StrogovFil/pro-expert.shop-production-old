	...
	BX.Sale.OrderAjaxComponent = {
		...
		showWarnings: function()
		{
			...
			for (k = 0; k < sections.length; k++)
			{
				...
				if (sections[k].getAttribute('data-visited') == 'false')
					BX.removeClass(sections[k], 'passed');
			}
			...
			if (currentDelivery && currentDelivery.CALCULATE_ERRORS)
			{
				...
				if (this.activeSectionId != this.deliveryBlockNode.id)
				{
					BX.addClass(this.deliveryBlockNode, 'passed');
		...
		locationsCompletion: function()
		{
			...
			if (this.activeSectionId !== this.regionBlockNode.id)
				this.editFadeRegionContent(this.regionBlockNode.querySelector('.order-row-body'));

			if (this.activeSectionId != this.propsBlockNode.id)
				this.editFadePropsContent(this.propsBlockNode.querySelector('.order-row-body'));
		},
		...
		showAuthBlock: function()
		{
			...
			if (!showNode || BX.hasClass(showNode, 'active'))
				return;
		...
		showActualBlock: function()
		{
			...
			while (allSections[i])
			{
				...
				BX.addClass(allSections[i], 'passed');
		...
		{
			return BX.create('DIV', {props: {className: 'order-row-body' + (!!notFluid ? '' : ' container-fluid')}});
		},
		...
		changeVisibleSection: function(section, state)
		{
			...
			{
				content = section.querySelector('.order-row-body');
				if (content)
		...
		editAuthBlock: function()
		{
			...
			var authContent = this.authBlockNode.querySelector('.order-row-body'),
			...
			if (this.result.OK_MESSAGE && this.result.OK_MESSAGE.length)
			{
				...
				BX.prepend(okMessageNode, this.authBlockNode.querySelector('.order-row-body'));
			}
		},
		...
		toggleAuthForm: function(event)
		{
			...
			var target = event.target || event.srcElement,
				...
				container = BX.findParent(target, {className: 'order-row-body'}),
		...
		editActiveBasketBlock: function(activeNodeMode)
		{
			...
			{
				basketContent = node.querySelector('.order-row-body');
		...
		editFadeBasketBlock: function(activeNodeMode)
		{
			...
			if (this.initialized.basket)
			{
				this.basketHiddenBlockNode.appendChild(node.querySelector('.order-row-body'));
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			{
				regionContent = node.querySelector('.order-row-body');
				if (!regionContent)
				{
		...
		{
			var regionContent = this.regionBlockNode.querySelector('.order-row-body'), newContent;
				...
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						currentType = this.result.PERSON_TYPE[i];
							...
							label = BX.create('LABEL', {
								props: {className: 'order-row-customertype-btn'},
								children: [
									BX.create('INPUT', {
		...
		editActivePaySystemBlock: function(activeNodeMode)
		{
			...
			{
				paySystemContent = node.querySelector('.order-row-body');
				if (!paySystemContent)
				{
				...
				paySystemNode = BX.create('DIV', {props: {className: 'order-row-squares'}});
		...
		{
			var paySystemContent = this.paySystemBlockNode.querySelector('.order-row-body'), newContent;
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var /*paySystemItemsContainer = BX.create('DIV', {props: {className: 'order-row-squares'}}),*/
				paySystemItemNode, i;
		...
		createPaySystemItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				title, label, itemNode, parentLogoNode, spanNode, inputNode;
			...
			parentLogoNode = BX.create('SPAN', {
				props: {className: 'order-row-squares-icon'},
				children: [logoNode]
			});
			
			if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == 'Y')
				title = BX.create('SPAN', {props: {className: 'order-row-squares-title'}, text: item.NAME});
			
			spanNode = BX.create('SPAN', {
				props: {className: 'order-row-squares-btn-inner'},
				children: [parentLogoNode, title]
			});
			
			inputNode = BX.create('INPUT', {
						props: {
			...
			itemNode = BX.create('LABEL', {
				props: {className: 'order-row-squares-btn'},
				children: [inputNode, spanNode],
				events: {
...
/*console.log(item);
		...
		selectPaySystem: function(event)
		{
			...
			var innerPsAction = BX.hasClass(target, 'bx-soa-pp-inner-ps') ? target : BX.findParent(target, {className: 'bx-soa-pp-inner-ps'}),
				actionSection = BX.hasClass(target, 'order-row-squares-btn') ? target : BX.findParent(target, {className: 'order-row-squares-btn'}),
			...
			else if (actionSection)
			{
				...
				{
					selectedSection = this.paySystemBlockNode.querySelector('.order-row-squares-btn.bx-selected');
					BX.addClass(actionSection, 'bx-selected');
					actionInput = actionSection.querySelector('input[type=checkbox]');
					actionInput.checked = true;
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			{
				deliveryContent = node.querySelector('.order-row-body');
				if (!deliveryContent)
				{
				...
				deliveryNode = BX.create('DIV', {props: {className: 'order-row-squares'}});
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var currentDelivery, extraServices, extraServicesNode;/*deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-sm-5 bx-soa-pp-desc-container'}}),
			...
			if (extraServices.length)
			{
				extraServicesNode = BX.create('DIV', {
					props: {className: 'order-row-addon'},
					children: extraServices
				});

				deliveryNode.parentNode.appendChild(extraServicesNode);
			}
		...
		getDeliveryExtraServices: function(delivery)
		{
			...
			for (i in delivery.EXTRA_SERVICES)
			{
				...
				else
				{
					...
					serviceNode = BX.create('LABEL', {
							props: {className: 'order-row-addon-btn'},
								html: currentService.editControl + '<span><i class="icon icon-plus"></i>' + currentService.priceFormatted + ' ' + BX.util.htmlspecialchars(currentService.name) + '</span>'
							});
				}
		...
		{
			var deliveryContent = this.deliveryBlockNode.querySelector('.order-row-body'), newContent;
		...
		createDeliveryItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, itemNode, logoNode, inputNode, spanNode, parentLogoNode, descriptionNode, priceNode;
//console.log(item);
			...
			if (this.params.SHOW_DELIVERY_LIST_NAMES == 'Y')
			{
				title = BX.create('SPAN', {
					props: {className: 'order-row-squares-title'},
					text: this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.NAME : item.OWN_NAME
				});
			}
			
			descriptionNode = BX.create('SPAN', {
					props: {className: 'order-row-squares-description'},
					text: item.DESCRIPTION
				});
			
			priceNode = BX.create('SPAN', {
					props: {className: 'order-row-squares-price'},
					text: item.PRICE_FORMATED
				});
			
			spanNode = BX.create('SPAN', {
				props: {className: 'order-row-squares-btn-inner'},
				children: [parentLogoNode, title, descriptionNode, priceNode]
			});
		...
		selectDelivery: function(event)
		{
			...
			var target = event.target || event.srcElement,
				actionSection =  BX.hasClass(target, 'order-row-squares-btn') ? target : BX.findParent(target, {className: 'order-row-squares-btn'}),
				selectedSection = this.deliveryBlockNode.querySelector('.order-row-squares-btn.bx-selected'),
		...
		activatePickUp: function(deliveryName)
		{
			...
			this.pickUpBlockNode.querySelector('div.bx-soa-section-title').innerHTML = BX.util.htmlspecialchars(deliveryName);
		...
		editActivePickUpBlock: function(activeNodeMode)
		{
			...
			{
				pickUpContent = node.querySelector('.order-row-body');
				if (!pickUpContent)
				{
		...
		{
			var pickUpContent = this.pickUpBlockNode.querySelector('.order-row-body'), newContent;
		...
		pickUpFinalAction: function()
		{
			...
			if (deliveryChanged && this.pickUpBlockNode.id !== this.activeSectionId)
			{
				...
				BX.removeClass(this.pickUpBlockNode, 'passed');
			}
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			{
				propsContent = node.querySelector('.order-row-body');
				if (!propsContent)
				{
		...
		{
			var propsContent = this.propsBlockNode.querySelector('.order-row-body'), newContent;
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'order-row-fields'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator;

			if (!propsItemsContainer)
				propsItemsContainer = this.propsBlockNode.querySelector('.order-row-fields');
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			else
			{
				...
				switch(settings.CODE)
				{
					...
					case 'EMAIL':
					case 'UF_STREET':
						className = "order-row-fields-3";
						break;
					case 'UF_HOUSE':
					case 'UF_APARTMENT':
					case 'UF_KORPUS':
						className = "order-row-fields-1";
						break;
					default:
						className = "order-row-fields-6";
				}
				...
				label = BX.create('LABEL', {
					...
					props: {className: 'form-label form-label-shorttitle form-input'},
					html: '<span class="form-label-title">' + textHtml + '</span>'
				});
			...
			switch (propertyType)
			{
				...
				case 'FILE':
					...
					break;
				case 'STRING':
					this.insertStringProperty(property, label, disabled);
					break;
				case 'ENUM':
				...
				case 'Y/N':
					...
					break;
				case 'NUMBER':
					this.insertNumberProperty(property, label, disabled);
			}
			propsItemNode.appendChild(label);
		...
		{
			var prop, inputs, values, i, propContainer, test;
		...
		showValidationResult: function(inputs, errors)
		{
			...
			var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
				formGroup = BX.findParent(input0, {tagName: 'DIV', className: 'order-row-fields'}),
			...
			for (i = 0; i < inputs.length; i++)
			{
				inputDiv = BX.findParent(inputs[i], {tagName: 'DIV', className: 'order-row-fields'});
		...
		{
			var propsCommentContainer, label, input, div, spanNode;

			propsCommentContainer = BX.create('DIV', {props: {className: 'order-row-fields'}});
			...
			spanNode = BX.create('SPAN', {
				props: {className: 'form-label-title'},
			...
			label = BX.create('LABEL', {
				...
				props: {className: 'form-label form-label-shorttitle form-input'},
				children: [input, spanNode] 
			});

			div = BX.create('DIV', {
				props: {className: 'order-row-fields-6'},
				children: [label]
			});
		...
		editTotalBlock: function()
		{
			...
			var total = this.result.TOTAL, priceNode,
				priceHtml, params = {},
			...
			if (this.options.showDiscountPrice)
				priceHtml += '<div class="order-summary-discount">Скидка ' + total.DISCOUNT_PRICE_FORMATED + '</div>';
				
			priceNode = BX.create('DIV', {
				props: {className: 'order-summary-items'},
				html: BX.message('SOA_SUM_SUMMARY') + ' ' + priceHtml
			});
			
			this.totalInfoBlockNode.appendChild(priceNode);
			...
			{
				priceNode = BX.create('DIV', {
					props: {className: 'order-summary-delivery'},
					html: BX.message('SOA_SUM_DELIVERY') + ' ' + deliveryValue
				});
			...
			priceNode = BX.create('DIV', {
				props: {className: 'order-summary-cost'},
				html: BX.message('SOA_SUM_IT') + ' ' + total.ORDER_TOTAL_PRICE_FORMATED
			});
			...
			if (!this.result.SHOW_AUTH)
			{
				this.totalInfoBlockNode.appendChild(
					BX.create('BUTTON', {
						props: {
							className: 'btn order-summary-btn'
						},
